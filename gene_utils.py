...
class GenomicTextLMDataBunch(TextLMDataBunch):
    ...
    def from_df(cls, path:PathOrStr, train_df:DataFrame, valid_df:DataFrame, test_df:Optional[DataFrame]=None,
                ...
                min_freq:int=10, mark_fields:bool=False, bptt=70, collate_fn:Callable=data_collate, bs=64, **kwargs):
...
class GenomicTextClasDataBunch(TextClasDataBunch):
    ...
    def from_df(cls, path:PathOrStr, train_df:DataFrame, valid_df:DataFrame, test_df:Optional[DataFrame]=None,
                tokenizer:Tokenizer=None, vocab:Vocab=None, classes:Collection[str]=None, text_cols:IntsOrStrs=1,
                label_cols:IntsOrStrs=0, label_delim:str=None, chunksize:int=10000, max_vocab:int=60000,
                min_freq:int=10, mark_fields:bool=False, pad_idx=0, pad_first=True, bs=64, **kwargs) -> DataBunch:
        "Create a `TextDataBunch` from DataFrames. `kwargs` are passed to the dataloader creation."
        processor = _get_genomic_processor(tokenizer=tokenizer, vocab=vocab, chunksize=chunksize, max_vocab=max_vocab,
                                   min_freq=min_freq, mark_fields=mark_fields)
        if classes is None and is_listy(label_cols) and len(label_cols) > 1: classes = label_cols
...
def get_scores(learn, ret=False):
    pr, y = learn.get_preds()
    p = torch.argmax(pr, dim=1)
    ...
    # auc_c = roc_auc_score(y.numpy(), p.numpy())
    # print(auc_c)
    # mauc = my_auc(y.numpy(), p.numpy())
    # print(mauc)
    ...
    False_Positives = fp/len(y)
    False_Negatives = fn/len(y)
    Recall = tp / (tp + fn)
    Precision = tp / (tp + fp)
    Specificity = tn / (tn + fp)

    res = [auc_c,Accuracy,False_Positives,False_Negatives,cc,Recall,Precision,Specificity]
    print(f'AUC: {auc_c}')
